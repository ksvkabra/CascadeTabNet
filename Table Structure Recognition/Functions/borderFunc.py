...
def extract_table(table_body, __line__, lines=None):
    ...
    for i, row in enumerate(points):
        ...
        for j, col in enumerate(row):
            ...
            if (i == 0):
                nextcol = row[j+1]
                lastCache.append([col[0], col[1], nextcol[0],
                                  nextcol[1], 9999, 9999, 9999, 9999])
            else:
                nextcol = row[j+1]
                currentVala.append(
                    [col[0], col[1], nextcol[0], nextcol[1], 9999, 9999, 9999, 9999])
                # Matching
                flag = 1
                index = []
    ...
    count = 1
    for i in box:
        cv2.rectangle(table_body, (i[0], i[1]), (i[6], i[7]), (int(
            i[7] % 255), 0, int(i[0] % 255)), 2)
    #     count+=1
